...
void Curvefit::init(int indata,int infit)
{ if ((error==107)||(error==103))
  ...
  return;
} // end of Curvefit::init

float Curvefit::getmemory()
{ float xa;
  if (error==0)
    xa=(float)(sizeof(Curvefit)+nfit*4*sizeof(int)+
      (ndata*4+nfit*7+nfit*nfit+nfit*ndata+1)*sizeof(float));
  else xa=-1.0f;
  return(xa);
} //end of Curvefit::getmemory

int Curvefit::loadcurve(int ifitmath,int itrymax,float itolerance,
...
{ int i,k;
  ...
  return(error);
} // end of Curvefit::loadcurve

int Curvefit::dofit(float *ybest,float *abest,float *reliability,
  int *lastrynum)
{ int i;
...
{ int i,ia,ib,ka,kb,kc,devstep;
  ...
  return(error);
} // end of Curvefit::pdangsearch

int Curvefit::netsearch()
{ int i,j,k,m,ia,ib,ic,ja,jb,jc,ka,kb,kc;
  ...
  if (error==0) for (i=0;i<nfit;++i) afit[i]=beta[i];
  return(error);
} // end of Curvefit::netsearch

int Curvefit::downsimplex()
{ int i,j,k,m,ilow,ihigh,inhigh,itst;
  float rtol,ftol,sum,temp;

  ftol=0.0f;
  if (error==0)
  { ftol=(float)sqrt(fabs((double)tolerance));
    for (m=0;m<nfit+1;++m)
    { if (m==0) i=0;
      else i=yafit[m-1]+1;
      if ((m==0)||(i>0))
  ...
  return(error);
} // end of Curvefit::downsimplex

int Curvefit::levenmarq()
{ int itst;
  float pchisq;
  ...
  return(error);
} // end of Curvefit::levenmarq

float Curvefit::downhill(int ihigh,float factor)
{ int j,k;
  float fac1,fac2;
  ...
  return(reliable);
} // end of Curvefit::downhill

int Curvefit::marqmin()
{ int i,j,k,p,q;
  ...
  return(error);
} // end of Curvefit::marqmin

int Curvefit::marqcof()
{ int i,j,k,m,p;
  float wt,dy;
  ...
  return(error);
} // end of Curvefit::marqcof

int Curvefit::gausjord(float *amat,float *bmat,int na,int nb)
