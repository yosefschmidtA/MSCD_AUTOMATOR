...
{ int ia,ib,ic,j,k,m,n,p,q,tscatter,analyze;
  ...
  analyze=0;
  Textout conout;
  //Abner here I change condition of natom=100 to 333 
  if (((error==0)&&(dispmode>1)&&(msorder>2)&&(natoms>20))||
    ((error==0)&&(dispmode>1)&&(msorder==2)&&(natoms>333))||
    ((error==0)&&(dispmode>4)))
  ...
  { ++analyze;
    ...
    { if ((msorder==2)&&(patom[ia*12+7]==0)) continue;
      ...
      { if (ib==ia) continue;
        ...
        akind=patom[ib*12+6]; lena=round(lena,0.005f); // Abner change 0.001 to 0.005
        ...
        { if (ic==ib) continue;
          ...
          cosbeta=round(cosbeta,0.005f); //Abner idem
          search=(float)((akind-1.0)*awidth+vlenb+cosbeta+1.005); // Abner idem
          search=round(search,0.005f); // Abner idem
  ...
  { if ((msorder==2)&&(patom[ia*12+7]==0)) continue;
    ...
    { if (ib==ia) continue;
      ...
      za=patom[ib*12+2]-patom[ia*12+2];
      lena=(float)sqrt(xa*xa+ya*ya+za*za);
      akind=patom[ib*12+6]; lena=round(lena,0.005f); //Abner idem
