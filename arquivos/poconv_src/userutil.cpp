...
Textout::Textout(const char *filename,_Ios_Openmode mode)
{ fout.open(filename,mode);//ofstream fout(filename,mode);
  if (!fout)
  { 
    cout<<filename;
    error=203; fdout=0;
...
int Textout::open(const char *filename,_Ios_Openmode mode)//,_Ios_Openmode mode)
{ if (fdout<1) fout.open(filename,mode);//,ios::out);//mode);
  if (!fout)
  { 
    error=203; fdout=0;
  }
